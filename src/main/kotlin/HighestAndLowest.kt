/*
    codewars challenge:
    https://www.codewars.com/kata/554b4ac871d6813a03000035/train/kotlin

    In this little assignment you are given a string of space separated numbers, and have to return the highest and lowest number.
    Examples
        highAndLow("1 2 3 4 5")  // return "5 1"
        highAndLow("1 2 -3 4 5") // return "5 -3"
        highAndLow("1 9 3 4 -5") // return "9 -5"
    Notes
        All numbers are valid Int32, no need to validate them.
        There will always be at least one number in the input string.
        Output string must be two numbers separated by a single space, and highest number is first.
 */
class HighestAndLowest {

    companion object {
        fun mySolution(numbers: String): String {
            val numbersWithoutSpace = numbers.split(" ")

            val numbersMap = mutableSetOf<Int>()

            for (t in numbersWithoutSpace) {
                numbersMap.add(t.toInt())
            }

            val sortedNumbersMap = numbersMap.sorted()
            return "${sortedNumbersMap.last()} ${sortedNumbersMap.first()}"
        }

        fun bestSolution(numbers: String): String {
            val x = numbers.split(" ").map { it.toInt() }.sorted()
            return "${x.last()} ${x.first()}"
        }
    }
}

fun main() {
    val numbers = "1 2 -3 42 5"

    println("Highest and Lowest, My Solution: ${HighestAndLowest.mySolution(numbers)}")
    println("Highest and Lowest Best Solution: ${HighestAndLowest.bestSolution(numbers)}")
}
